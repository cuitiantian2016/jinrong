...
import com.xulu.mpush.api.ack.AckCallback;
import com.xulu.mpush.api.ack.AckContext;
import com.xulu.mpush.api.connection.SessionContext;
import com.xulu.mpush.api.connection.SessionStorage;
import com.xulu.mpush.api.http.HttpRequest;
import com.xulu.mpush.api.http.HttpResponse;
import com.xulu.mpush.api.protocol.Command;
import com.xulu.mpush.api.protocol.Packet;
import com.xulu.mpush.api.push.PushContext;
import com.xulu.mpush.handler.HttpProxyHandler;
import com.xulu.mpush.message.AckMessage;
import com.xulu.mpush.message.BindUserMessage;
import com.xulu.mpush.message.FastConnectMessage;
import com.xulu.mpush.message.HandshakeMessage;
import com.xulu.mpush.message.HttpRequestMessage;
import com.xulu.mpush.message.PushMessage;
import com.xulu.mpush.message.RequestMarketMessage;
import com.xulu.mpush.security.AesCipher;
import com.xulu.mpush.security.CipherBox;
import com.xulu.mpush.session.PersistentSession;
import com.xulu.mpush.util.Strings;
...
public final class MPushClient implements Client, AckCallback {
    ...
    public void onNetStateChange(boolean isConnected) {
        ...
        if (isConnected) { //当有网络时，去尝试重连
            ...
            hbTimeoutTimes = Constants.MAX_HB_TIMEOUT_COUNT - 2;//总共要调用两次healthCheck，第一次用于发送心跳，第二次用于检测是否超时
    ...
    public boolean healthCheck() {
        ...
        if (hbTimeoutTimes >= Constants.MAX_HB_TIMEOUT_COUNT) {
            logger.w("heartbeat timeout times=%d over limit=%d, client restart", hbTimeoutTimes, Constants.MAX_HB_TIMEOUT_COUNT);
            hbTimeoutTimes = 0;
...
//        context.changeCipher(CipherBox.INSTANCE.getRsaCipher());
        ...
        message.clientKey = CipherBox.INSTANCE.aESKey();
        message.iv = CipherBox.INSTANCE.aESKey();
        message.deviceId = config.getDeviceId();
