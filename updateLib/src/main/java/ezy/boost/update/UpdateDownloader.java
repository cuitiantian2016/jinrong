...
class UpdateDownloader extends AsyncTask<Void, Integer, Long> {
    ...
    @Override
    protected void onProgressUpdate(Integer... progress) {//同步更新UI
        switch (progress[0]) {
        case EVENT_START:
            mAgent.onStart();
            break;
    ...
    @Override
    protected void onPostExecute(Long result) {//超过结束
        mAgent.onFinish();
    }
    ...
    private long download() throws IOException, UpdateError {
        ...
        RandomAccessFile file = new LoadingRandomAccessFile(mTemp);
        InputStream stream = mConnection.getInputStream();
        int bytesCopied = copy(stream, file);
