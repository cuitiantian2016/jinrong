...
public class Tool {
    ...
    public final static String get32MD5Str(String str) {
        ...
        try {
            ...
            LogUtils.logd("Tool", "NoSuchAlgorithmException:" + e.toString());
        } catch (UnsupportedEncodingException e) {
            LogUtils.logd("Tool", "UnsupportedEncodingException:" + e.toString());
        }
    ...
    public static List<String> toList(JSONArray arr) {
        ...
        for (int i = 0; i < arr.length(); i++) {
            try {
                ...
                LogUtils.logd("Tool", "Failed To List");
                return str_list;
            }
        }
        return str_list;
    ...
    public static void showLog(String tag, String result) {
        ...
        if (result.length() < length) {
            LogUtils.logd(tag, result);
        } else {
            int multiple = result.length() / length;
            for (int i = 0; i < multiple; i++) {
                start = i * length;
                end = i * length + length;
                LogUtils.logd(tag, result.substring(start, end));
            }
            LogUtils.logd(tag, result.substring(result.length() - result.length() % length, result.length()));
        }
    }
     ...
     * 加载网络图片
     */
    public static void setImage(Activity mActivity, String url, ImageView imageView) {
        Glide.with(mActivity).load(url)//设置图片并且去除换成防止下次加载显示上张图片禁用磁盘缓存
                .placeholder(R.drawable.image_default_new)//设置图片并且去除换成防止下次加载显示上张图片禁用磁盘缓存
                .error(R.drawable.image_default_new)
                .centerCrop()
                .into(imageView);
    }

    public static void setImage(Activity mActivity, File url, ImageView imageView) {
