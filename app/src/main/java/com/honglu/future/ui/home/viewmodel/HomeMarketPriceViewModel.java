...
public class HomeMarketPriceViewModel extends IBaseView<MarketData> implements View.OnClickListener {
    ...
    private void refreshData() {
        if (marketPresenter ==null){
            marketPresenter = new BasePresenter<IBaseView<MarketData>>(this){
                ...
                public void getData() {
                    super.getData();
                    toSubscribe(HttpManager.getApi().getMarketData(), new HttpSubscriber<MarketData>() {
                        @Override
    ...
    public void bindData(MarketData marketData) {
        if (marketData==null||marketData.getData()==null||marketData.getData().getData()==null)return;
        List<MarketData.DataBeanX.DataBean.DataListBean> dataList = marketData.getData().getData().getDataList();
        if (arrayList!=null){
            arrayList.clear();
        }else {
