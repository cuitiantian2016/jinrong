            ...
            if (TradeRecordPresenter.TYPE_HISTORY_TRADE.equals(type)){
                mHandler.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        ...
                        mPresenter.getHistoryTradeBean(startTime, SpUtil.getString(Constant.CACHE_TAG_UID),SpUtil.getString(Constant.CACHE_ACCOUNT_TOKEN),TimeUtil.getStringByOffset(endTime,TimeUtil.dateFormatYMD,Calendar.DATE,1));
                    }
                },1000);
            }else {
                getHistoryData(clickId);
