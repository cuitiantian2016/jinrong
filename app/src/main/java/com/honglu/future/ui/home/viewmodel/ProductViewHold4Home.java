...
public class ProductViewHold4Home {
    public class ProductView {
        ...
        public String tag;
        public MarketData.MarketDataBean optional;
    }
    ...
    public ProductViewHold4Home(List<View> mListView, Context context, String codes) {
     ...
     * @param optional
     */
    public void updateProductViewListDisplay(MarketData.MarketDataBean optional) {
        if (optional == null)
            return;
        ...
        if (productView.tv_product != null) {
            if (!TextUtils.isEmpty(optional.name)) {
                productView.tv_product.setText(optional.name);
            }
        }
        if (productView.tv_pro_open != null)
            productView.tv_pro_open.setText(optional.lastPrice);
        if (productView.tv_pro_rate != null) {

            double diff = Double.parseDouble(optional.change);
            //格式化小数点
            String diffStr = optional.change;
            String diffPercentStr = optional.chg;
        ...
        checkChange(productView.rl_product, optional.getCode(), Double.parseDouble(optional.lastPrice));
     ...
     * 控件初始化
     */
   private void initProductViewList(String codes) {
