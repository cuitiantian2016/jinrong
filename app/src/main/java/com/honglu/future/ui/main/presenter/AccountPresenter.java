        ...
        toSubscribe(HttpManager.getApi().loginAccount(account, AESUtils.encrypt(password), userId, company), new HttpSubscriber<AccountBean>() {
            ...
            protected void _onError(String message, int code) {
                ...
                if (code == 70000) {//首次登录
                    tv_pass.setText("");
                    SpUtil.putString(Constant.CACHE_ACCOUNT_USER_NAME, account);
                    new AlertFragmentDialog.Builder((FragmentActivity) App.mApp.mActivity)
                            .setLeftBtnText("取消").setContent(message, R.color.color_333333, R.dimen.dimen_25sp).setTitle("", R.color.color_3C383F, R.dimen.dimen_16sp)
