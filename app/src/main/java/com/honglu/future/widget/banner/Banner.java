...
public class Banner extends LinearLayout {
    ...
    public void changeIndicatorStyle(int count, int height, int color) {
		int widthDp = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, count* DeviceUtils.dip2px(getContext(),6),
                mIndicatorTitle.getContext().getResources()
				.getDisplayMetrics())+DeviceUtils.dip2px(getContext(),12);
      ...
      }else if (mRule == Rule.RIGHT){
          params.addRule(RelativeLayout.ALIGN_PARENT_RIGHT, RelativeLayout.TRUE);
      }else if (mRule == Rule.CENTER){
          params.addRule(RelativeLayout.CENTER_HORIZONTAL, RelativeLayout.TRUE);
      }

        params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
        params.setMargins(0,0,DeviceUtils.dip2px(getContext(),12), DeviceUtils.dip2px(getContext(),6));
