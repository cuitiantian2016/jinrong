...
import com.honglu.future.util.NumberType;
...
public class RiseNumberTextView extends TextView {
    ...
    private RealTimeBean.Data optional;// 旨在格式化数据

    public RealTimeBean.Data getOptional() {
        return optional;
    }

    public void setOptional(RealTimeBean.Data optional) {
        this.optional = optional;
        try {
            int format = ProFormatConfig.getProFormatMap(optional.getInstrumentID());
            if (format <= 0) {
                //保留自己的小数点位数
                format = NumberUtils.getPointPow(optional.getLastPrice());
            }
