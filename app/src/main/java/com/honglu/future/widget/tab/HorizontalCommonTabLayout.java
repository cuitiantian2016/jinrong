...
public class HorizontalCommonTabLayout extends HorizontalScrollView implements ValueAnimator.AnimatorUpdateListener {
    ...
    private boolean mIndicatorGravityBlue;
    ...
    private String mSpecialUnSelectTxt = "";
    private int mSpecialUnSelectColor;
    ...
    public HorizontalCommonTabLayout(Context context) {
        this(context, null, 0);
    }

    public HorizontalCommonTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public HorizontalCommonTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        try {
            mIsSingleLine = ta.getBoolean(R.styleable.CommonTabLayout_tl_indicator_single_line,true);
            mSpecialUnSelectTxt = ta.getString(R.styleable.CommonTabLayout_tl_special_un_select_contains_txt);
            mSpecialUnSelectColor = ta.getColor(R.styleable.CommonTabLayout_tl_special_un_select_color,Color.parseColor("#B2B2B2"));
        }catch (Exception e){
           // LogUtils.e(e.getMessage());
        }

        mUnderlineColor = ta.getColor(R.styleable.CommonTabLayout_tl_underline_color, Color.parseColor("#ffffff"));
        mIndicatorGravityBlue = ta.getBoolean(R.styleable.CommonTabLayout_tl_special_select_color_blue,false);
    ...
    public void updateTabData(ArrayList<CustomTabEntity> tabEntitys){
        if (tabEntitys == null || tabEntitys.size() == 0) {
            throw new IllegalStateException("TabEntitys can not be NULL or EMPTY !");
        }
        this.mTabEntitys.clear();
        this.mTabEntitys.addAll(tabEntitys);
        updateTabTextContents();
    }

    private void updateTabTextContents() {
        for (int i = 0; i < mTabCount; i++) {
            View tabView = mTabsContainer.getChildAt(i);
            TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
            try {
                if (mTabEntitys != null && mTabEntitys.size() > 0) {
                    CustomTabEntity entity = mTabEntitys.get(i);
                    String preContent = tv_tab_title.getText().toString();
                    if (entity != null && !TextUtils.equals(preContent,entity.getTabTitle())) {
                        tv_tab_title.setText(entity.getTabTitle());
                    }
                }
            }catch (Exception e){
             //   LogUtils.e(e.getMessage());
    ...
    private void addTab(final int position, View tabView) {
        TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
        tv_tab_title.setSingleLine(mIsSingleLine);
    ...
    private void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
            int unSelectColor = mTextUnselectColor;
            try {
                if (!TextUtils.isEmpty(mSpecialUnSelectTxt)){
                    String currentText = tv_tab_title.getText().toString().trim();
                    if (!TextUtils.isEmpty(currentText)){
                        if (currentText.contains(mSpecialUnSelectTxt)){
                            unSelectColor = mSpecialUnSelectColor;
                        }
                    }
                }
            }catch (Exception e){
                unSelectColor = mTextUnselectColor;
            }

            tv_tab_title.setTextColor(i == mCurrentTab ? mTextSelectColor : unSelectColor);
...
//            tv_tab_title.setPadding((int) mTabPadding, 0, (int) mTabPadding, 0);
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            try {
                if (!TextUtils.isEmpty(mSpecialUnSelectTxt)){
                    String currentText = tab_title.getText().toString().trim();
                    if (!TextUtils.isEmpty(currentText)){
                        if (currentText.contains(mSpecialUnSelectTxt)){
                            unSelectColor = mSpecialUnSelectColor;
                        }
            ...
            tab_title.setTextColor(isSelect ? mTextSelectColor : unSelectColor);
            ImageView iv_tab_icon = (ImageView) tabView.findViewById(R.id.iv_tab_icon);
            CustomTabEntity tabEntity = mTabEntitys.get(i);
