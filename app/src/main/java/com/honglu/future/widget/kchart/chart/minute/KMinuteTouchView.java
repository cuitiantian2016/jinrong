package com.honglu.future.widget.kchart.chart.minute;
...
//                    canvas.drawLine(x, axisYtopHeight, x, getHeight() - axisYbottomHeight - axisYmiddleHeight, crossPaint);
                //竖线 时间在下面
                if (showSubChart)
                    canvas.drawLine(x, axisYbottomHeight, x, getHeight(), crossPaint);
                else
                    canvas.drawLine(x, axisYbottomHeight, x, getHeight() - axisYtopHeight - axisYmiddleHeight, crossPaint);
                //左边价格width
                float w = textPaint.measureText(KNumberUtil.beautifulDouble(fenshiData.getClose(),numberScal));
...
//                        right, axisYtopHeight);
                RectF rectFT = new RectF(left, getDataHeightMian() + axisYtopHeight,
                        right, getDataHeightMian() + axisYtopHeight + axisYtopHeight);
                //左边价格矩形
                ...
                //画出文字 价格
                drawText(canvas, KNumberUtil.beautifulDouble(fenshiData.getClose(),numberScal), rectF, textPaint);
                //画出文字 百分比
                ...
                if (showSubChart) {
                    drawCrossVol(canvas,fenshiData);
                }
...
//                canvas.drawText(fenshiData.getTime(), rectFT.left + (rectFT.right - rectFT.left)/2 - wT / 2, axisYtopHeight / 2 + latitudeFontSize / 2, textPaintT);
     ...
     * @param canvas
     */
    public void drawCrossVol(Canvas canvas,KCandleObj obj) {
        //时间paint
        Paint textPaintT = getPaint();
        textPaintT.setColor(longitudeFontColor);
        textPaintT.setTextSize(latitudeFontSize);
        textPaintT.setStyle(Paint.Style.FILL);

        //成交量
        String maxv = KNumberUtil.formartBigNumber(maxVol) + "";//手
        float w = textPaintT.measureText(maxv);
        canvas.drawText("VOL="+obj.getVol(), axisXleftWidth*2+w, mainF * getHeight() + latitudeFontSize, textPaintT);
